    ...
    namespace misc_utils_ns
{
    ...
    void PublishCloud(const ros::Publisher& cloud_publisher, const PCLPointCloudType& cloud, const std::string& frame_id)
    {
        ...
        pcl::toROSMsg(cloud, cloud_msg);//把PCLPointCloudType类型的数据转换成sensor_msgs,数据从cloud输入，转成成msgs之后存到cloud_msgs
    ...
    public:
        static int id_;
        visualization_msgs::Marker marker_;//在使用ROS进行机器人控制的时候，有时候会希望能够显示并且保留机器人的历史运动轨迹，比如最近在做行人跟踪，就希望能够保留多个行人的轨迹以及机器人的运动。
        //每次获取到机器人的位姿后，就在对应点发布一个标志，然后将lifetime设为0，也就是无限久地保存～～但是需要注意一点，让ns或者id变量每次都不一样，否则ns和id一直一样的话，后面的操作会覆盖前面的操作，也就一直只能看到最新的了。建议每次让id+=1。
