...
//类 VoxelGrid 根据给定的点云构造一个三维体素栅格并进行下采样达到滤波的效果。具体说来类 VoxelGrid 通过输入的点云数据创建一个三维体素栅格 ， 然后将每个体素内所有的点都用
//该体素内的点集的重心来近似，这样就大大减少了数据量。所以该类常用于对大数据量的下采样处理，特别是在配准、曲面重建等工作之前作为预处理．可以很好地提高程序的速度。 
//示例：
        //pcl::VoxelGrid<pcl::PCLPointCloud2> sor;// 创建滤波器对象
        //sor.setInputCloud(cloud);
        //sor.setLeafSize(0.01f, 0.01f, 0.01f);//设置滤波时创建的体素体积为 1cm*1cm*1cm的立方体
        //sor.filter(*cloud_filtered);//把cloud经过滤波后的结果存入*cloud_filtered
